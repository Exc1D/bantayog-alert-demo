const ALERT_COLORS = {
  watch: { bg: 'bg-amber-50', border: 'border-amber-400', text: 'text-amber-800', icon: '\u{1F7E1}' },
    ...
    return (
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-bold text-emerald-800 text-xs uppercase tracking-wider">No Active Weather Alerts</p>
            <p className="text-[10px] text-emerald-600 mt-0.5">Conditions are normal across the province</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-2">
      {alerts.map((alert, index) => {
        ...
        return (
          <div
            key={index}
            className={`${colors.bg} border-l-4 ${colors.border} rounded-r-xl p-3`}
          >
