  ...
  return (
    <div className="grid grid-cols-2 gap-3 p-4" role="radiogroup" aria-label="Disaster type">
      {DISASTER_TYPES.map(({ id, label, icon: Icon }) => (
        ...
        >
          <Icon size={28} weight={selected === id ? 'fill' : 'regular'} className={selected === id ? 'text-emergency dark:text-emergency-dark' : 'text-text-muted-dark dark:text-text-muted-dark'} aria-hidden="true" />
          <span className={`text-sm font-medium ${selected === id ? 'text-emergency dark:text-emergency-dark' : 'text-text-dark dark:text-text-dark'}`}>{label}</span>
        </button>
      ))}
    </div>
  );
