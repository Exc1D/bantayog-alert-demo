  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 relative bg-dark-bg dark:bg-dark-bg flex flex-col items-center justify-center p-4">
        <input type="file" accept="image/*" capture="environment" onChange={handleCapture} className="absolute inset-0 opacity-0 cursor-pointer" aria-label="Capture photo" />
        {!photoFile ? (
          ...
          <div className="relative w-full h-full">
            ...
            <button type="button" onClick={() => onPhotoSelect(null)} className="absolute top-2 right-2 w-8 h-8 bg-[#1C1C1E] text-white rounded-full flex items-center justify-center text-sm" aria-label="Remove photo">×</button>
          </div>
        )}
      </div>
      <div className="p-4 flex gap-3">
        <button type="button" onClick={() => {}} className="flex-1 py-3 rounded-xl border border-dark-border text-dark-text dark:text-dark-text flex items-center justify-center gap-2 text-sm">
