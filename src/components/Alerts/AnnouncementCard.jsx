...
const AnnouncementCard = memo(function AnnouncementCard({
  ...
  return (
    ...
    >
      ...
      {/* Body */}
      {body && <p className="text-xs opacity-90 mb-3 leading-relaxed">{body}</p>}

      {/* Footer: scope badge + time ago */}
      <div className="flex items-center justify-between gap-2 mt-auto">
        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${scopeStyle}`}>
          {scope}
        </span>
        <span className="text-[10px] opacity-70">{formatTimeAgo(createdAt)}</span>
      </div>
    </div>
  );
});
