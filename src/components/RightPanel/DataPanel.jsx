...
export default function DataPanel() {
  ...
  const cutoff = Date.now() / 1000 - periodSec;
  const filtered = reports.filter(r => !r.createdAt?.seconds || r.createdAt.seconds >= cutoff);
  ...
  const municipalityCounts = useMemo(() => {
    const counts = {};
    filtered.forEach(r => { counts[r.municipality ?? 'Unknown'] = (counts[r.municipality ?? 'Unknown'] ?? 0) + 1; });
  ...
  return (
    <div className="h-full overflow-y-auto p-3 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1.5">
        <h3 className="text-xs font-semibold text-text-muted-dark flex items-center gap-1"><MapPin size={14} aria-hidden="true" />By Municipality</h3>
        {municipalityCounts.length === 0 ? <p className="text-xs text-text-muted-dark italic">No data for this period</p> :
          municipalityCounts.map(([name, count]) => (
            <div key={name} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-4 bg-surface-dark rounded-full overflow-hidden">
                <div className="h-full bg-emergency/70 dark:bg-emergency-dark/70 rounded-full transition-all" style={{ width: `${(count / maxCount) * 100}%` }} />
              </div>
      ...
      <div className="flex items-center gap-2 p-3 bg-surface-dark/30 rounded-lg border border-border-dark">
        ...
        <span className="text-xs text-text-muted-dark">{filtered.length} reports in the last {period}</span>
      </div>
    </div>
  );
}
