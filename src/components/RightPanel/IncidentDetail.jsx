...
export default function IncidentDetail() {
  ...
  return (
    <div className="flex flex-col h-full bg-dark-bg dark:bg-dark-bg overflow-hidden">
      <div className="flex items-center gap-3 p-3 border-b border-dark-border flex-shrink-0">
        <button
          type="button"
          onClick={handleBack}
          className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-dark-bg/50 text-muted-dark hover:text-dark-text transition-colors"
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
        ...
        {report.status === 'resolved' && report.resolutionNote && (
          <div className="mt-2 p-3 bg-safe/10 border-l-2 border-safe rounded">
            ...
            <p className="text-sm text-dark-text dark:text-dark-text">{report.resolutionNote}</p>
          </div>
        )}
      </div>
    </div>
