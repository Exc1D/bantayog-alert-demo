...
import {
  Warning, MapPin, Clock, ArrowLeft, CheckCircle,
    ...
    <div className="flex flex-col h-full bg-surface-dark dark:bg-surface-dark overflow-hidden">
      ...
      <div className="flex items-center gap-3 p-3 border-b border-border-dark flex-shrink-0">
        ...
        <div className="flex-1">
          <h2 className="text-sm font-bold text-text-dark capitalize">{report.disaster?.type ?? 'Incident'}</h2>
          <p className="text-xs text-text-muted-dark">{report.municipality}</p>
        </div>
        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${isResolved ? 'bg-safe/20 text-safe' : sevColor}`}>
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            {report.location && <p className="text-xs text-text-muted-dark font-mono">{report.location.lat?.toFixed(5)}, {report.location.lng?.toFixed(5)}</p>}
          </div>
        </div>
        ...
        {report.disaster?.description && <p className="text-sm text-text-dark leading-relaxed">{report.disaster.description}</p>}
        ...
        {isResolved && report.resolutionNote && (
          <div className="mt-2 p-3 bg-safe/10 border-l-2 border-safe rounded">
            <p className="text-xs font-semibold text-safe flex items-center gap-1 mb-1">
              <CheckCircle size={14} aria-hidden="true" />Resolution
            </p>
