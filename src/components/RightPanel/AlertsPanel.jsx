...
function AlertItem({ report, userLat, userLng, onSelect }) {
  const dist = userLat && userLng ? distanceKm(userLat, userLng, report.lat, report.lng).toFixed(1) : null;
  return (
    <button
      type="button"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-text-dark dark:text-text-dark capitalize">{report.disaster?.type} — {report.municipality}</p>
        <p className="text-xs text-text-muted-dark dark:text-text-muted-dark mt-0.5 line-clamp-2">{report.disaster?.description ?? `Severity: ${report.disaster?.severity}`}</p>
        <div className="flex items-center gap-2 mt-1">
          {dist && <span className="text-xs text-text-muted-dark dark:text-text-muted-dark">{dist} km away</span>}
