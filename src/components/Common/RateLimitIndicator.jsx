...
export default function RateLimitIndicator({
  ...
  if (compact) {
    return (
      <div className={`flex items-center gap-1.5 text-xs ${style.text}`}>
        ...
        {resetTime > 0 && <span className="opacity-75">(resets in {formatTime(resetTime)})</span>}
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-lg border p-3 ${style.container}`}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-sm font-medium ${style.text}`}>
            {isAllowed ? `${remainingAttempts} attempts remaining` : 'Rate limit reached'}
          </span>
        </div>
        {resetTime > 0 && (
      ...
      {message && <p className={`mt-2 text-xs ${style.text}`}>{message}</p>}

      {!isAllowed && (
        <p className="mt-2 text-xs text-gray-600 italic">
          This is a client-side limit. For abuse prevention, server-side rate limiting is
          recommended.
        </p>
      )}
    </div>
